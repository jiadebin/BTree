...
int main(int argc, char **argv)
{
  ...
  try
	...
	if (testNum == 6){
		...
		BTreeIndex index1(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
		deleteRelation();
		createRelationForward();
		BTreeIndex index2(relationName, doubleIndexName, bufMgr, offsetof(tuple,d), DOUBLE);
		deleteRelation();
		createRelationForward();
		BTreeIndex index3(relationName, stringIndexName, bufMgr, offsetof(tuple,s), STRING);
		deleteRelation();
		printf("all 3 indexes has been created\n");
		return 1 ;
	}
...
void test1()
{
	...
	indexTests();
	deleteRelation();
	printf("passed createRelationForward()\n");
}
...
void test2()
{
	...
	indexTests();
	deleteRelation();
	printf("passed createRelationBackward()\n");
}
...
void test3()
{
	...
	indexTests();
	deleteRelation();
	printf("passed createRelationRandom()\n");
}

void test7(){
	// Create a relation with tuples valued 0 to 10000 and perform index tests 
	// on attributes of all three types (int, double, string)
	std::cout << "---------------------" << std::endl;
	std::cout << "createRelationAlot" << std::endl;
	createRelationAlot();
	indexTests();
	deleteRelation();
	printf("passed createRelationAlot()\n");
}
...
void createRelationRandom()
{
  ...
  /*
  for (int xx = 0; xx < relationSize; xx++) {
  	if (arr[abs(arr[xx])] >= 0)
      arr[abs(arr[xx])] = -arr[abs(arr[xx])];
    else
      printf("dup: %d\n", abs(arr[xx]));
  }
  */
...
void createRelationAlot()
{
  ...
  // Insert 10000 tuples into the relation.
...
void doubleTests()
{
  ...
  BTreeIndex index(relationName, doubleIndexName, bufMgr, offsetof(tuple,d), DOUBLE);
	...
	//additonal tests
	checkPassFail(doubleScan(&index,-1000,GT,6000,LT), 5000)
}
...
void stringTests()
{
  ...
  BTreeIndex index(relationName, stringIndexName, bufMgr, offsetof(tuple,s), STRING);
	...
	//additonal tests
	checkPassFail(stringScan(&index,-1000,GT,6000,LT), 5000)
}
